...
def classifier(): #to predict if a event has a dwell time above the median
    ...
    def cross_val_LR():
        model_MR = LogisticRegression(max_iter = 1000)
        cross_validation = KFold(n_splits = 5, shuffle = True, random_state = 32)  #splits test/training automatically
        scores = cross_val_score(model_MR, x_data_array, y_data_array, cv = cross_validation, scoring = "accuracy")  #fits automatically
        print(f"LR: {scores}")

    def cross_val_DT():
        model_DT = DecisionTreeClassifier()
        cross_validation = KFold(n_splits = 5, shuffle = True, random_state = 32)  #splits test/training automatically
        scores = cross_val_score(model_DT, x_data_array, y_data_array, cv = cross_validation, scoring = "accuracy")  #fits automatically
        print(f"DT: {scores}")

    def cross_val_RF():
        model_RF = RandomForestClassifier(random_state = 32)
        cross_validation = KFold(n_splits = 5, shuffle = True, random_state = 32)  #splits test/training automatically
        scores = cross_val_score(model_RF, x_data_array, y_data_array, cv = cross_validation, scoring = "accuracy")  #fits automatically
        print(f"RF: {scores}")

    def saving_model():
        model = LogisticRegression(max_iter = 1000) #or DecisionTreeClassifier(max_depth = . . .)
        joblib.dump(model, "classifier_model.joblib" )
        joblib.dump(encoder, "brand_encoder.joblib")
...
def clustering():
    ...
    session_features = df.groupby("session_id").agg(
        mean_dwell = ("dwell_seconds", "mean"),
        mean_confidence = ("confidence", "mean"),
        unique_brands = ("brand", "nunique"),
        n_events = ("brand", "count")
    )

    pipe = make_pipeline(StandardScaler(), KMeans(n_clusters = 5)) #make_pipeline executes the validation of KMean, Standard Scaler makes everything on the same scale so one variable doesn't outweigh all others.
    output = pipe.fit_predict(session_features)
    print(output)
